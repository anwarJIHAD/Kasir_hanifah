...
class Kembali1 extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $this->load->view('kembali/vw_kembali1', $data);
        $this->load->view('layout/footer', $data);
    }

    public function transaksi1($id)
    {
        ...
        $data['role'] = $this->db->get_where('user', ['id_sales' => $this->session->userdata('id_sales')])->row_array();
        $this->load->view('layout/header', $data);
        $this->load->view('kembali/vw_kembali2', $data);
        $this->load->view('layout/footer', $data);
    }
    public function export($id)
    {
        $dompdf = new Dompdf();
        $this->data['transaksi2'] = $this->Detail_model->get($id);
        $this->data['barang'] = $this->Barang_model->get();
        $this->data['id_transaksi'] = $this->Transaksi_model->getById($id);
        $this->data['sales'] = $this->Detail_model->getSales($id);
        $this->data['tgl'] = $this->Transaksi_model->getById($id);
        $this->data['edtran'] = $this->Detail_model->getById($id);
        $this->data['user'] = $this->db->get_where('sales', ['id_sales' => $this->session->userdata('id_sales')])->row_array();
        $this->data['role'] = $this->db->get_where('user', ['id_sales' => $this->session->userdata('id_sales')])->row_array();
        $this->data['judul'] = 'Laporan Data Penjualan';
        $this->data['no'] = 1;
        $dompdf->setPaper('A4', 'Portrait');
        $dompdf->set_option('isRemoteEnabled', true);
        $html = $this->load->view('report', $this->data, true);
    ...
    public function transaksi2($id)
    {
        ...
        $data['role'] = $this->db->get_where('user', ['id_sales' => $this->session->userdata('id_sales')])->row_array();
        $this->load->view('layout/header', $data);
        $this->load->view('kembali/vw_kembali3', $data);
        $this->load->view('layout/footer', $data);
    }
